          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b dark:border-slate-800">
                <th className="pb-4 font-bold text-xs uppercase tracking-wider text-slate-500">Lead Info</th>
                <th className="pb-4 font-bold text-xs uppercase tracking-wider text-slate-500">Company</th>
                <th className="pb-4 font-bold text-xs uppercase tracking-wider text-slate-500">Status</th>
                <th className="pb-4 font-bold text-xs uppercase tracking-wider text-slate-500">Source</th>
                <th className="pb-4 font-bold text-xs uppercase tracking-wider text-slate-500">Assigned To</th>
                <th className="pb-4 font-bold text-xs uppercase tracking-wider text-slate-500 text-right">Actions</th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                ...
                leadsResponse?.data.map((lead: ILead) => (
                  <tr key={lead._id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group">
                    ...
                    <td className="py-4 text-sm font-medium">{lead.company}</td>
                    <td className="py-4">
                    ...
                    <td className="py-4 text-sm text-slate-600 dark:text-slate-400 font-medium">{lead.source}</td>
                    <td className="py-4">
                      <div className="text-sm font-bold text-primary-600">{lead.assignedTo?.name || 'Unassigned'}</div>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">Created {formatDate(lead.createdAt)}</div>
                    </td>
                    <td className="py-4 text-right">
                      <div className="flex items-center justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleAction(lead, 'view')} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg text-slate-600" title="View"><Eye size={16} /></button>
                        <button onClick={() => handleAction(lead, 'edit')} className="p-2 hover:bg-blue-50 text-blue-600 dark:hover:bg-blue-900/20 rounded-lg" title="Edit"><Edit2 size={16} /></button>
                        {user?.role === 'Admin' && (
                          <button onClick={() => handleAction(lead, 'delete')} className="p-2 hover:bg-red-50 text-red-600 dark:hover:bg-red-900/20 rounded-lg" title="Delete"><Trash2 size={16} /></button>
                        )}
                      </div>
                    </td>
                  </tr>
          ...
          <div className="flex items-center justify-between pt-4 border-t dark:border-slate-800">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-bold px-4">
                Page {leadsResponse.pagination.page} of {leadsResponse.pagination.totalPages}
              </span>
              <button 
